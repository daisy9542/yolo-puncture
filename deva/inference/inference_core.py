from typing import List, Optional, Dict, Literal, Iterable, Tuple
import warnings
...
class DEVAInferenceCore:
    def __init__(self,
        ...
        self.forward_clip_frames = config['forward_clip_frames']
    ...
    def _add_memory(self,
                    ...
                    *,
                    ...
                    yolo_features: Tuple[torch.Tensor, torch.Tensor, torch.Tensor] = None,
                    ) -> None:
        # image: 1*3*H*W
    ...
    def clear_buffer(self) -> None:
        ...
        for f in self.frame_buffer:
            if f.ti <= self.curr_ti - self.forward_clip_frames:
                self.image_feature_store.delete(f.ti)
        # self.frame_buffer = []
        for i in range(len(self.frame_buffer) - 1, -1, -1):
            if self.frame_buffer[i].ti <= self.curr_ti - self.forward_clip_frames:
                del self.frame_buffer[i]
    ...
    def incorporate_detection(self,
        ...
        yolo_features = self.image_feature_store.get_yolo_features(image_ti)
        ...
        self._add_memory(image, ms_features, self.last_mask, key, shrinkage, selection, yolo_features=yolo_features)
